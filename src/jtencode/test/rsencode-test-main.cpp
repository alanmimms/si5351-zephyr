...
int main() {
  ...
  try {
    RSEncode encode1(3, 0xB, 1, 1, 4, 0);
    std::vector<uint8_t> data1 = {0x01, 0x02, 0x03};
    std::vector<uint8_t> parity1;
  ...
  try {
    ...
    encode2.encode(emptyData, parity2);
  ...
  try {
    RSEncode encode3(4, 0x13, 0, 1, 5, 2);
    std::vector<uint8_t> data3 = {0xA1, 0xA2, 0xA3, 0xA4, 0xA5, 0xA6, 0xA7, 0xA8};
  ...
  try {
    RSEncode encode4(3, 0xB, 1, 1, 4, 0);
    std::vector<uint8_t> longData = {0x10, 0x11, 0x12, 0x13, 0x14};
    std::vector<uint8_t> parity4;

    encode4.encode(longData, parity4);
  ...
  try {
    RSEncode encode5(3, 0xB, 1, 1, 4, 0);
    std::vector<uint8_t> shortData = {0x0A};
    std::vector<uint8_t> parity5;

    encode5.encode(shortData, parity5);
  ...
  try {
    ...
    RSEncode invalidEncode(0, 0xB, 1, 1, 4, 0);
  ...
  try {
    ...
    RSEncode invalidEncode(3, 0xB, -1, 1, 4, 0);
    std::cout << "ERROR: Expected exception, but none thrown." << std::endl;
  } catch (const std::runtime_error& e) {
    std::cout << "Caught expected exception: " << e.what() << std::endl;
  }
  ...
  try {
    RSEncode invalidEncode(3, 0x7, 1, 1, 4, 0);
  ...
  try {
    ...
    RSEncode invalidEncode(3, 0xB, 1, 0, 4, 0);
    std::cout << "ERROR: Expected exception, but none thrown." << std::endl;
  } catch (const std::runtime_error& e) {
    std::cout << "Caught expected exception: " << e.what() << std::endl;
  }
  try {
    ...
    RSEncode invalidEncode(3, 0xB, 1, 1, 9, 0);
    std::cout << "ERROR: Expected exception, but none thrown." << std::endl;
  } catch (const std::runtime_error& e) {
    std::cout << "Caught expected exception: " << e.what() << std::endl;
  }
  try {
    ...
    RSEncode invalidEncode(3, 0xB, 1, 1, 4, 5);
    std::cout << "ERROR: Expected exception, but none thrown." << std::endl;
  } catch (const std::runtime_error& e) {
    std::cout << "Caught expected exception: " << e.what() << std::endl;
  }
