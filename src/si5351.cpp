...
SI5351::SI5351(uint32_t clkinHz,
	       PLLSource pllA = pllSrcXTAL,
	       PLLSource pllB = pllSrcXTAL,
	       CLKINDivider clkin = div1,
	       )
    :  clockHz(clkinHz)
{
  for (int k=0; k < nChannels; ++k) driveStrength[k] = ds20mA;

  reset(pllA, pllB, clkin);
}


SI5351::reset(PLLSource pllA = pllSrcXTAL, PLLSource pllB = pllSrcXTAL, CLKINDivider clkin = div1) {

  // Disable clock outputs.
  i2cWrite(clk0_3Disable, 0xFF);
  i2cWrite(clk4_7Disable, 0xFF);

  // Cripple OEB pins so they cannot enable our clocks.
  i2cWrite(OEBPinDisable, 0xFF);

  // Power down all output drivers.
  for (int k=0; k < 8; ++k) i2cWrite(clkControlBase+k, 0x80);

  // Write current frequency and phase configuration.

  // Reset PLLs.
  i2cWrite(PLLReset, 0xA0);

  // Disable icky spread spectrum.
  i2cWrite(ssEnable, 0);

  // Enable desired outputs CLK0..CLK2.
  i2cWrite(outputDisable, 0xFF ^ 0x07);
}
